...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
  ...
  console.error(
    "Please set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY (or SUPABASE_ANON_KEY) in .env file",
  );
  process.exit(1);
}
...
async function runMigration() {
  console.log(
    "🔄 Running migration: Add crop_type and related columns to farms table...\n",
  );
  ...
  try {
    ...
    if (testError && testError.message.includes("crop_type")) {
      console.log(
        "📋 Column crop_type does not exist. Please run the following SQL in Supabase Dashboard:\n",
      );
...
`);
      ...
      if (fullTestError) {
        ...
        console.log(
          "✅ All required columns exist (crop_type, season, water_source, irrigation_type)",
        );
        console.log(
