...
export function applyOfflineDriftToLatestReading(
  ...
  const rainMm =
    typeof context.rainMmNext6h === "number" ? context.rainMmNext6h : 0;
  ...
  if (cropName.includes("rice"))
    cropMoistureFactor = 0.85; // usually irrigated/puddled
  else if (cropName.includes("sugarcane")) cropMoistureFactor = 1.1;
  ...
  const tempFactor = Number.isFinite(tempC)
    ? clamp(1 + (tempC - 25) * 0.03, 0.6, 2.0)
    : 1.0;

  // If rain is likely soon, reduce moisture decay (or slightly increase if a lot of rain expected)
  const rainMoistureFactor = rainLikely
    ? clamp(1 - Math.min(0.7, 0.15 + rainMm * 0.05), 0.25, 1.0)
    : 1.0;
  ...
  const rainNutrientFactor = rainLikely
    ? clamp(1 + Math.min(0.6, 0.2 + rainMm * 0.08), 1.0, 1.6)
    : 1.0;
  const dryNutrientFactor =
    !rainLikely &&
    Number.isFinite(Number(latest.soil_moisture)) &&
  ...
  phosphorusRate =
    phosphorusRate * (rainNutrientFactor * 0.8 + 0.2) * dryNutrientFactor;
  ...
  if (Number.isFinite(soilMoisture)) {
    adjusted.soil_moisture = clamp(
      expDecay(soilMoisture, moistureRate, hoursElapsed),
      0,
      100,
    );
  }
  ...
  if (Number.isFinite(nitrogen)) {
    adjusted.nitrogen = clamp(
      expDecay(nitrogen, nitrogenRate, hoursElapsed),
      0,
      500,
    );
  }
  ...
  if (Number.isFinite(phosphorus)) {
    adjusted.phosphorus = clamp(
      expDecay(phosphorus, phosphorusRate, hoursElapsed),
      0,
      500,
    );
  }
  ...
  if (Number.isFinite(potassium)) {
    adjusted.potassium = clamp(
      expDecay(potassium, potassiumRate, hoursElapsed),
      0,
      500,
    );
  }
  ...
  const logs = Array.isArray(context.actionLogsSinceLastReading)
    ? context.actionLogsSinceLastReading
    : [];
  const actionOf = (l: ActionLogRow) =>
    (l.action_type || l.action || "").toLowerCase();
  const irrigationCount = logs.filter(
    (l) => actionOf(l) === "irrigation",
  ).length;
  const fertilizationCount = logs.filter(
    (l) => actionOf(l) === "fertilization",
  ).length;
  ...
  if (fertilizationCount > 0) {
    ...
    if (Number.isFinite(curN))
      adjusted.nitrogen = clamp(curN + fertilizationCount * nBump, 0, 500);

    const curP = Number(adjusted.phosphorus);
    if (Number.isFinite(curP))
      adjusted.phosphorus = clamp(curP + fertilizationCount * pBump, 0, 500);

    const curK = Number(adjusted.potassium);
    if (Number.isFinite(curK))
      adjusted.potassium = clamp(curK + fertilizationCount * kBump, 0, 500);
  }
