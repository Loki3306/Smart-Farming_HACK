...
export async function detectPlantStress(
  ...
  if (!PLANTID_API_KEY) {
    throw new Error(
      "PLANTID_API_KEY is not configured. Sign up at https://web.plant.id/",
    );
  }
  ...
  const diseases = (data.result.disease.suggestions || []).map((disease) => {
    ...
    let type: "disease" | "nutrient_deficiency" | "water_stress" | "pest" =
      "disease";
    const diseaseName = disease.name.toLowerCase();

    if (
      diseaseName.includes("nitrogen") ||
      diseaseName.includes("deficiency") ||
      diseaseName.includes("nutrient") ||
      diseaseName.includes("chlorosis")
    ) {
      type = "nutrient_deficiency";
    } else if (
      diseaseName.includes("water") ||
      diseaseName.includes("drought") ||
      diseaseName.includes("wilting")
    ) {
      type = "water_stress";
    } else if (
      diseaseName.includes("pest") ||
      diseaseName.includes("insect") ||
      diseaseName.includes("aphid") ||
      diseaseName.includes("mite")
    ) {
      type = "pest";
    }
...
export async function quickStressCheck(imageBuffer: Buffer): Promise<{
  ...
  try {
    ...
    const topDisease = result.diseases.sort(
