  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [imageAnalysis, satelliteData] = await Promise.all([
        ...
        polygonId
          ? getVegetationHealth(polygonId).catch(() => null)
      ...
      if (satelliteData) {
        if (
          satelliteData.severity === "critical" ||
          satelliteData.severity === "stressed"
        ) {
      ...
      imageAnalysis.diseases.forEach((disease) => {
        if (disease.probability > 0.5) {
          recommendations.push(
            `${disease.name}: ${disease.treatment[0] || "Monitor closely"}`,
          );
        }
      });
  ...
  try {
    ...
    if (!farmId || !latitude || !longitude || !areaAcres) {
      return res.status(400).json({
        error:
          "Missing required fields: farmId, latitude, longitude, areaAcres",
      });
    }
