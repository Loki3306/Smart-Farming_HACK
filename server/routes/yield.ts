...
export const createYieldRecord = async (req: Request, res: Response) => {
  try {
    ...
    if (!yieldData.farmer_id || !yieldData.crop_type) {
      return res
        .status(400)
        .json({ error: "farmer_id and crop_type are required" });
    }
    ...
    if (yieldData.sensor_snapshot) {
      try {
        ...
        console.log(
          "[Yield] Could not get ML prediction, using provided values",
        );
      }
    }
...
export const logHarvest = async (req: Request, res: Response) => {
  try {
    ...
    if (yieldRecord.predicted_yield_kg && yieldRecord.actual_yield_kg) {
      const diff = Math.abs(
        yieldRecord.predicted_yield_kg - yieldRecord.actual_yield_kg,
      );
      accuracy = Math.max(
        0,
        100 - (diff / yieldRecord.predicted_yield_kg) * 100,
      );
    }
  ...
  try {
    ...
    const comparisons = (result.data || []).map((record: any) => {
      ...
      const accuracy = Math.max(
        0,
        100 - (Math.abs(diff) / record.predicted_yield_kg) * 100,
      );
    ...
    const avgAccuracy =
      totalRecords > 0
        ? comparisons.reduce(
            (sum: number, c: any) => sum + c.accuracy_percent,
            0,
          ) / totalRecords
        : 0;
    const betterThanPredicted = comparisons.filter(
      (c: any) => c.performed_better,
    ).length;
...
export const getYieldHistory = async (req: Request, res: Response) => {
  try {
    ...
    history.forEach((record: any) => {
      ...
      byCrop[crop].best_yield = Math.max(
        byCrop[crop].best_yield,
        record.actual_yield_kg,
      );
      byCrop[crop].worst_yield = Math.min(
        byCrop[crop].worst_yield,
        record.actual_yield_kg,
      );
    });
    ...
    Object.keys(byCrop).forEach((crop) => {
      byCrop[crop].avg_yield = Math.round(
        byCrop[crop].total_yield / byCrop[crop].count,
      );
...
function generateFallbackPrediction(data: any) {
  const cropBenchmarks: {
    [key: string]: { avg: number; min: number; max: number };
  } = {
  ...
  const yieldPotential =
    ((baseYield - benchmark.min) / (benchmark.max - benchmark.min)) * 100;
