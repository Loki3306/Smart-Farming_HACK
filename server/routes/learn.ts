...
class MockQuery implements PromiseLike<any> {
  ...
  select(cols: string = '*', opts?: any) { this.selects = cols; return this; }
  eq(col?: string, val?: any) { if (col) this.eqMap[col] = val; return this; }
...
const updateUserStats = async (userId: string) => {
  try {
    ...
    const totalSeconds =
      progress?.reduce((sum, p) => sum + (p.time_spent_seconds || 300), 0) || 0;
...
router.get("/courses", async (req: Request, res: Response) => {
  try {
    ...
    if (search) {
      query = query.or(
        `title.ilike.%${search}%,description.ilike.%${search}%,instructor_name.ilike.%${search}%`,
      );
    }
...
router.get("/courses/:id", async (req: Request, res: Response) => {
  try {
    ...
    if (lessonsError)
      return errorResponse(res, 400, "Failed to fetch lessons", lessonsError);
...
router.post("/courses", checkAuth, async (req: Request, res: Response) => {
  try {
    const { title, description, category, level, duration, language, price } =
      req.body;
    ...
    if (!title || !description || !category || !level) {
      return errorResponse(
        res,
        400,
        "Missing required fields: title, description, category, level",
      );
    }
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (error)
        return errorResponse(res, 400, "Failed to delete course", error);
    ...
    try {
      ...
      if (!title || !content_type) {
        return errorResponse(
          res,
          400,
          "Missing required fields: title, content_type",
        );
      }
      ...
      if (error)
        return errorResponse(res, 400, "Failed to create lesson", error);
    ...
    try {
      ...
      if (!content) {
        for (const [key, value] of Object.entries(LESSON_RICH_CONTENT)) {
          ...
          if (
            normalizedTitle.includes(keyLower) ||
            keyLower.includes(normalizedTitle)
          ) {
            content = value;
            matchedKey = key;
            break;
      ...
      console.log(
        `[Lesson Content] "${lesson.title}" → Matched: "${matchedKey || "none"}"`,
      );
...
router.get("/articles", async (req: Request, res: Response) => {
  try {
    ...
    if (search) {
      query = query.or(
        `title.ilike.%${search}%,excerpt.ilike.%${search}%,content.ilike.%${search}%,author_name.ilike.%${search}%`,
      );
    }
    ...
    if (error)
      return errorResponse(res, 500, "Failed to fetch articles", error);
...
router.post("/articles", checkAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (error)
      return errorResponse(res, 400, "Failed to create article", error);
...
router.put("/articles/:id", checkAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (error)
      return errorResponse(res, 400, "Failed to update article", error);
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (error)
        return errorResponse(res, 400, "Failed to delete article", error);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (fetchError)
        return errorResponse(res, 404, "Article not found", fetchError);
      ...
      if (error)
        return errorResponse(res, 400, "Failed to like article", error);
...
router.get("/videos", async (req: Request, res: Response) => {
  try {
    ...
    if (search) {
      query = query.or(
        `title.ilike.%${search}%,description.ilike.%${search}%,creator_name.ilike.%${search}%`,
      );
    }
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (fetchError)
        return errorResponse(res, 404, "Video not found", fetchError);
  ...
  try {
    ...
    if (!course_id || !title) {
      return errorResponse(
        res,
        400,
        "Missing required fields: course_id, title",
      );
    }
    ...
    try {
      ...
      for (const answer of answers) {
        ...
        const isCorrect = options.some(
          (opt: any) => opt.is_correct && opt.text === answer.user_answer,
        );
...
router.get("/enrollments", checkAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (error)
      return errorResponse(res, 400, "Failed to fetch enrollments", error);
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (error)
        return errorResponse(res, 400, "Failed to update enrollment", error);
    ...
    try {
      ...
      if (!isFullyCompleted) {
        return errorResponse(
          res,
          400,
          `Complete all lessons first (${completedCount}/${totalLessons})`,
        );
      }
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (error)
        return errorResponse(res, 400, "Failed to update progress", error);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const completedLessons =
        progress?.filter((p: any) => p.status === "completed") || [];
...
router.get("/progress", checkAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (error)
      return errorResponse(res, 400, "Failed to fetch progress", error);
...
router.get("/roadmaps", async (req: Request, res: Response) => {
  try {
    ...
    if (error)
      return errorResponse(res, 400, "Failed to fetch roadmaps", error);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (error)
        return errorResponse(res, 400, "Failed to start roadmap", error);
...
router.post("/purchases", checkAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (error)
      return errorResponse(res, 400, "Failed to create purchase", error);
...
router.get("/purchases", checkAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (error)
      return errorResponse(res, 400, "Failed to fetch purchases", error);
...
router.put("/purchases/:id", checkAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (error)
      return errorResponse(res, 400, "Failed to update purchase", error);
  ...
  try {
    ...
    return successResponse(
      res,
      {
        ...
        total_points: finalQuestions.reduce(
      ...
      return successResponse(
        res,
        {
          ...
          attempts_remaining: quiz.max_attempts
            ? quiz.max_attempts - (attemptCount || 0) - 1
            : null,
        },
    ...
    try {
      ...
      if (attemptError || !attempt) {
        return errorResponse(
          res,
          400,
          "Invalid or already completed attempt",
          attemptError,
        );
      }
      ...
      for (const question of questions) {
        ...
        if (
          ...
          isCorrect =
            correctOption &&
            userAnswer?.user_answer?.toLowerCase().trim() ===
      ...
      if (passed) {
        ...
        if (
          ...
          time_spent_seconds < quiz.time_limit_minutes * 30
        ) {
      ...
      return successResponse(
        res,
        {
          ...
          results: quiz.show_correct_answer
            ? results
            : results.map((r: any) => ({
                ...r,
                explanation: null,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const bestAttempt = (attempts || []).reduce((best: any, curr: any) => {
        if (!best || (curr.percentage || 0) > (best.percentage || 0))
          return curr;
        return best;
      }, null);
    ...
    try {
      ...
      const detailedResults = (questions || []).map((question: any) => {
        ...
        const selectedOption = question.quiz_options.find(
          (opt: any) => opt.id === userAnswer?.selected_option_id,
        );
        const correctOption = question.quiz_options.find(
          (opt: any) => opt.is_correct,
        );
      ...
      return successResponse(
        res,
        {
          ...
          summary: {
            ...
            correct_answers: detailedResults.filter(
              (r: any) => r.user_answer.is_correct,
            ).length,
            score: attempt.score,
            max_score: attempt.max_score,
