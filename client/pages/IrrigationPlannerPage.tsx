...
const IrrigationPlannerPage: React.FC = () => {
  ...
  >([]);
  const [expandedMethod, setExpandedMethod] = useState<IrrigationMethod | null>(
    null,
  );
  ...
  const handleRegionChange = (regionCode: string) => {
    ...
    const regionName = availableRegions.find(
      (r) => r.code === regionCode,
    )?.name;

    if (activeTab === "saved") {
      toast(
        `Region changed to ${regionName}. Switch to "Create New Plan" to see updated prices.`,
        {
          icon: "💡",
          duration: 4000,
        },
    ...
    try {
      ...
      if (selectedWaterSources.length > 1) {
        const additionalSources = selectedWaterSources
          ...
          .join(", ");
        (plan as any).notes =
          `Multiple sources available: ${primarySource.name} (primary), ${additionalSources} (backup)`;
      }
  ...
  const handleDeletePlan = (planId: string) => {
    if (
      window.confirm("Are you sure you want to delete this irrigation plan?")
    ) {
  ...
  const filteredPlans = savedPlans.filter((plan) => {
    const matchesSearch =
      plan.sectionName.toLowerCase().includes(searchQuery.toLowerCase()) ||
      plan.waterSourceName.toLowerCase().includes(searchQuery.toLowerCase()) ||
      getMethodDisplayName(plan.method as IrrigationMethod)
        .toLowerCase()
      ...
      <div ref={containerRef} className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="mb-6">
          ...
          {(() => {
            ...
            const diff =
              region && region.material !== 1.0
                ? ((region.material - 1) * 100).toFixed(0)
                : null;
            return (
              <div className="flex items-center gap-2 mt-3 text-sm">
                ...
                {diff && (
                  <span
                    className={`px-3 py-1.5 rounded-full font-medium ${Number(diff) > 0 ? "bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400" : "bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400"}`}
                  >
                    {Number(diff) > 0 ? "+" : ""}
                    {diff}% vs base
                  </span>
          ...
          <div className="animate-card opacity-0 bg-white dark:bg-gray-800 rounded-3xl p-8 md:p-12 text-center shadow-sm">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-6 max-w-md mx-auto">
              ...
              {waterSources.length === 0 &&
                "Add water sources to start planning."}
            </p>
            <button
        ...
        {activeTab === "create" &&
          ...
          waterSources.length > 0 && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-500 dark:text-gray-400">
                            {section.area.toFixed(1)} acres •{" "}
                            {section.cropType || "No crop"}
                          </p>
                        </div>
                        {selectedSection?.id === section.id && (
              ...
              <div
                className={`animate-card opacity-0 bg-white dark:bg-gray-800 rounded-3xl shadow-sm p-6 hover:shadow-lg transition-all duration-300 ${!selectedSection ? "opacity-60" : ""}`}
              >
                <div className="flex items-center gap-3 mb-5">
                  <div
                  ...
                  <div className="flex-1">
                    <h2
                      className={`text-lg font-semibold ${!selectedSection ? "text-gray-400" : "text-gray-800 dark:text-white"}`}
                    >
                      Water Sources
                    </h2>
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                  ...
                  <>
                    ...
                    <div className="space-y-2 max-h-72 overflow-y-auto smooth-scroll pr-2">
                      {waterSources
                        ...
                        .map(({ source, distance }) => {
                          const isSelected = selectedWaterSources.some(
                            (s) => s.id === source.id,
                          );
                          const isPrimary =
                            selectedWaterSources[0]?.id === source.id;
                          return (
                            <div
                              key={source.id}
              ...
              <div
                className={`animate-card opacity-0 bg-white dark:bg-gray-800 rounded-3xl shadow-sm p-6 hover:shadow-lg transition-all duration-300 ${selectedWaterSources.length === 0 ? "opacity-60" : ""}`}
              >
                <div className="flex items-center gap-3 mb-5">
                  <div
                    className={`w-10 h-10 rounded-2xl flex items-center justify-center font-bold shadow-lg ${
                  ...
                  <div>
                    <h2
                      className={`text-lg font-semibold ${selectedWaterSources.length === 0 ? "text-gray-400" : "text-gray-800 dark:text-white"}`}
                    >
                      Smart Picks
                    </h2>
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                          ...
                          >
                            ...
                            >
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                  <div
                                    className={`text-xl font-bold ${getScoreColor(rec.suitabilityScore)}`}
                                  >
          ...
          <div className="space-y-4">
            ...
            <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/30 rounded-2xl p-4 text-sm text-amber-800 dark:text-amber-300 animate-fade-in">
              <p className="flex items-center gap-2">
                ...
                <span>
                  <strong>Note:</strong> Saved plans show prices from when they
                  were created (historical pricing). Region selector only
                  affects new plans. To see current regional prices, create a
                  new plan.
                </span>
              </p>
            </div>
            ...
            {savedPlans.length === 0 ? (
              <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-12 text-center animate-fade-in">
                ...
                <p className="text-gray-500 dark:text-gray-400 mb-6 max-w-xs mx-auto">
                  Create your first irrigation plan to get started
                </p>
                <button
                  onClick={() => setActiveTab("create")}
                            ...
                            <div className="flex items-center gap-4 flex-1">
                              ...
                              <div className="flex-1">
                                <div className="flex items-center gap-3">
                                  ...
                                  <span
                                    className={`px-3 py-1 rounded-full text-xs font-semibold capitalize ${getStatusColor(plan.status)}`}
                                  >
                                    {plan.status.replace("-", " ")}
                                  </span>
                                </div>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                            <div className="text-center p-3 bg-white dark:bg-gray-800 rounded-xl hover:shadow-md transition-all hover:-translate-y-0.5">
                              ...
                              <p className="font-semibold text-gray-800 dark:text-white text-sm">
                                {getMethodDisplayName(
                                  plan.method as IrrigationMethod,
                                )}
                              </p>
                            </div>
                            <div className="text-center p-3 bg-white dark:bg-gray-800 rounded-xl hover:shadow-md transition-all hover:-translate-y-0.5">
                            ...
                            <div className="text-center p-3 bg-white dark:bg-gray-800 rounded-xl hover:shadow-md transition-all hover:-translate-y-0.5">
                              ...
                              <p className="font-semibold text-gray-800 dark:text-white text-sm">
                                {(plan.waterRequirement / 1000).toFixed(1)}K L
                              </p>
                            </div>
                            <div className="text-center p-3 bg-white dark:bg-gray-800 rounded-xl hover:shadow-md transition-all hover:-translate-y-0.5">
                              <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">
                              ...
                              <p
                                className={`font-bold text-sm ${getScoreColor(plan.suitabilityScore)}`}
                              >
                              ...
                              <div className="grid grid-cols-3 gap-4">
                                ...
                                <div className="text-center p-4 bg-gradient-to-br from-emerald-50 to-emerald-100/50 dark:from-emerald-900/30 dark:to-emerald-800/20 border border-emerald-200/50 dark:border-emerald-700/30 rounded-xl hover:scale-[1.02] transition-transform">
                                  ...
                                  <p className="font-bold text-emerald-900 dark:text-emerald-100 text-lg">
                                    {formatCurrency(plan.estimatedCost.labor)}
                                  </p>
                                </div>
                                <div className="text-center p-4 bg-gradient-to-br from-amber-50 to-amber-100/50 dark:from-amber-900/30 dark:to-amber-800/20 border border-amber-200/50 dark:border-amber-700/30 rounded-xl hover:scale-[1.02] transition-transform">
                                  ...
                                  <p className="font-bold text-amber-900 dark:text-amber-100 text-lg">
                                    {formatCurrency(
                                      plan.estimatedCost.operational,
                                    )}
                                  </p>
                                </div>
                              </div>
                                  ...
                                  <tbody>
                                    {plan.components.map((component, idx) => (
                                      ...
                                      >
                                        ...
                                        <td className="text-right p-3 font-semibold text-gray-800 dark:text-white">
      ...
      {showLayoutMap &&
        ...
        (() => {
          ...
          const recommendation = recommendations.find(
            (r) => r.method === expandedMethod,
          );

          // Calculate pipe length from components
          const pipeComponent = recommendation?.components.find(
            (c) =>
              c.name.toLowerCase().includes("pipe") ||
              c.name.toLowerCase().includes("lateral"),
          );
          const pipeLength = pipeComponent
            ? pipeComponent.quantity
            : primarySource.distance * 1.2;
