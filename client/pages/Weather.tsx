    ...
    const fetchWeather = async () => {
      ...
      try {
        ...
        setCurrentWeather({
          ...
          sunrise: sunriseTime.toLocaleTimeString(i18n.language, {
            hour: "2-digit",
            minute: "2-digit",
            hour12: false,
          }),
          sunset: sunsetTime.toLocaleTimeString(i18n.language, {
            hour: "2-digit",
            minute: "2-digit",
            hour12: false,
          }),
        ...
        const formattedForecast: WeatherDay[] = dailyForecast.map(
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-xl opacity-90">
                  {getLocalizedCondition(currentWeather.condition)}
                </p>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="font-semibold">
                    {currentWeather.uvIndex} ({t("current.high")})
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
              ...
              <Card className="p-4">
                <div className="grid grid-cols-[1fr_auto_auto_auto] items-center gap-2 sm:gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-muted-foreground hidden sm:block w-24 truncate">
                      {getLocalizedCondition(day.condition)}
                    </span>
                  </div>
      ...
      >
        ...
        <ul className="space-y-2 text-sm text-muted-foreground">
          {forecast.length > 0 &&
            forecast.some((day) => day.precipitation > 50) && (
              <li>
                •{" "}
                <strong>
                  {t("insights.rainExpected", {
                    day: forecast.find((day) => day.precipitation > 50)?.day,
                  })}
                </strong>
              </li>
            )}
          {currentWeather.uvIndex >= 6 && (
            <li>
              • <strong>{t("insights.highUV")}</strong>
            </li>
          )}
          {currentWeather.windSpeed < 15 && (
            <li>
              • <strong>{t("insights.lowWind")}</strong>
            </li>
          )}
          {currentWeather.windSpeed >= 15 && (
            <li>
              • <strong>{t("insights.highWind")}</strong>
            </li>
          )}
          {forecast.length > 0 &&
            forecast.some((day) => day.precipitation > 50) && (
              <li>
                • <strong>{t("insights.soilMoisture")}</strong>
              </li>
            )}
          {currentWeather.temperature > 30 && (
            <li>
              • <strong>{t("insights.highTemp")}</strong>
            </li>
          )}
          {currentWeather.humidity < 40 && (
            <li>
              • <strong>{t("insights.lowHumidity")}</strong>
            </li>
          )}
          {currentWeather.humidity > 80 && (
            <li>
              • <strong>{t("insights.highHumidity")}</strong>
            </li>
          )}
        </ul>
      </Card>
