...
export const IrrigationLayoutMap: React.FC<IrrigationLayoutMapProps> = ({
  ...
  useEffect(() => {
    ...
    const sectionCentroid: [number, number] = [
    ...
    setTimeout(() => {
      ...
      const sectionCoords = coords.map(
    ...
    `);
      ...
      const waterSourceMarker = L.marker(
    ...
    `);
      ...
      L.marker(sectionCentroid, { icon: centroidIcon }).addTo(map).bindPopup(`
        <div class="text-sm">
    ...
    `);
      ...
      for (let i = 1; i <= numArrows; i++) {
        ...
        const arrowLat =
          ...
          (sectionCentroid[0] - waterSource.coordinates[0]) * fraction;
        const arrowLng =
          waterSource.coordinates[1] +
          (sectionCentroid[1] - waterSource.coordinates[1]) * fraction;
  ...
  const addDripIrrigationPattern = (
    map: L.Map,
    section: SectionData,
    ...
    const numLines = Math.max(
      5,
      Math.min(20, Math.floor((latDiff * 111000) / lineSpacing)),
    );
    ...
    for (let i = 0; i < numLines; i++) {
      ...
      const lat =
        ...
        (bounds.getNorthEast().lat - bounds.getSouthWest().lat) * fraction;
      ...
      if (linePoints.length > 1) {
        ...
        for (let k = 1; k <= linePoints.length; k++) {
          if (
            k === linePoints.length ||
            Math.abs(linePoints[k][1] - linePoints[k - 1][1]) > 0.0002
          ) {
            if (k - segmentStart > 1) {
  ...
  const addSprinklerPattern = (
    ...
    const numRows = Math.max(
      2,
      Math.min(6, Math.floor((latDiff * 111000) / sprinklerSpacing)),
    );
    const numCols = Math.max(
      ...
      Math.min(
        6,
        Math.floor(
          (lngDiff * 111000 * Math.cos((centroid[0] * Math.PI) / 180)) /
            sprinklerSpacing,
        ),
      ),
    );
