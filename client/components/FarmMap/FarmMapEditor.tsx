...
import {
  ...
  type FarmSection,
  type GeoJSONPolygon,
} from "../../services/farmGeometryService";
...
export const FarmMapEditor: React.FC<FarmMapEditorProps> = ({
  ...
  const selectedSection =
    externalSelectedSection !== undefined ? externalSelectedSection : null;
  ...
  const loadFarmSections = async () => {
    try {
      const loadedSections = await farmGeometryService.listSections(
        farmId,
        true,
      );
  ...
  const handleBoundaryDrawn = async (
    ...
    try {
      const updatedGeometry = await farmGeometryService.updateFarmBoundary(
        farmId,
        geometry,
      );
      ...
      toast.success(
        `Farm boundary saved! Area: ${updatedGeometry.area_acres?.toFixed(2)} acres`,
      );
      ...
      toast.error(
        error.response?.data?.detail || "Failed to save farm boundary",
      );
  ...
  const handleSectionDrawn = async (
    geometry: GeoJSONPolygon,
    layer: L.Polygon,
  ) => {
    try {
      ...
      toast.success(
        `Section created: ${newSection.area_acres?.toFixed(2)} acres`,
      );
  ...
  const updateSection = async (sectionId: string, geometry: GeoJSONPolygon) => {
    try {
      const updatedSection = await farmGeometryService.updateSection(
        farmId,
        sectionId,
        {
          section_geojson: geometry,
      ...
      setSections(
        sections.map((s) => (s.section_id === sectionId ? updatedSection : s)),
      );
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        if (mapInstanceRef.current) {
          ...
          const marker = L.marker(location, { icon: userIcon }).addTo(
            mapInstanceRef.current,
          );
          marker.bindPopup(
        ...
        {farmGeometry?.has_geometry && (
          <div className="mb-4 p-3 bg-sky-50 rounded">
            ...
            <p className="text-xs text-gray-600">
              ...
              {farmGeometry.area_sq_meters?.toFixed(0)} m²)
            </p>
          </div>
        )}
