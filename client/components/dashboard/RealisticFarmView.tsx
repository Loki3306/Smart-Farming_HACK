  ...
  const SoilBlock = ({ id, delay }: { id: number; delay: number }) => {
    ...
    return (
      ...
      >
        {/* The 3D Soil Surface */}
        <div
          className={`absolute inset-0 rounded-xl ${soilColor} shadow-[inset_0_2px_10px_rgba(0,0,0,0.4)] border-t border-white/10 overflow-hidden transform transition-transform group-hover:-translate-y-1`}
        >
          ...
          {isDry && (
            <svg
              className="absolute inset-0 w-full h-full opacity-30 mix-blend-multiply"
              preserveAspectRatio="none"
            >
              <path
                d="M10,10 L30,40 M60,10 L40,50 M80,20 L70,80"
                stroke="#78350f"
                strokeWidth="1"
              />
            </svg>
    ...
    <div className="w-full p-1">
      ...
      <div className="relative rounded-2xl overflow-hidden shadow-xl bg-gradient-to-b from-sky-300 via-sky-100 to-white dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 border border-white/20 h-72 group">
        ...
        <div className="absolute inset-0 pointer-events-none">
          ...
          {/* Rain Overlay if wet */}
          {isWet && (
            <div className="absolute inset-0 bg-slate-900/10 z-20 backdrop-blur-[1px]"></div>
          )}
        </div>
        ...
        <div className="absolute top-3 left-3 right-3 z-30 flex justify-between">
          ...
          <div
            className={`backdrop-blur-md px-3 py-1 rounded-full h-fit flex items-center gap-2 shadow-lg border ${
        ...
        <div className="flex gap-3 items-center">
          ...
          {isDry && (
            <span className="flex items-center gap-1 text-xs text-amber-500">
              <Wind size={12} /> Needs Water
            </span>
          )}
          {!isWet && !isDry && (
            <span className="flex items-center gap-1 text-xs text-green-500">
              <Sprout size={12} /> Optimal
            </span>
          )}
