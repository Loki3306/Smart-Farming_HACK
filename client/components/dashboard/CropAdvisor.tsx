        ...
        <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 border border-gray-100">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <div className="flex-1">
                    <h2 className="text-xl sm:text-2xl font-bold text-gray-800 flex items-center gap-2">
                        ...
                        <span className="hidden sm:inline">Strategic Crop Advisor</span>
                        <span className="sm:hidden">Crop Advisor</span>
                    </h2>
                    <p className="text-xs sm:text-sm text-gray-500 mt-1">AI-driven biological & econometrics engine</p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-4 w-full sm:w-auto">
                    ...
                    <button
                        onClick={runAnalysis}
                        disabled={loading}
                        className={`px-4 sm:px-6 py-2.5 sm:py-2 rounded-xl font-bold text-sm sm:text-base text-white transition-all transform active:scale-95 sm:hover:scale-105 ${loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-green-700 hover:bg-green-800 shadow-md'
                            }`}
                    >
            ...
            {rec && (
                ...
                >
                    ...
                    <div className={`p-4 sm:p-6 rounded-2xl border-l-4 sm:border-l-8 ${rec.economic_switch ? 'bg-green-50 border-green-500' : 'bg-green-50 border-green-500'
                        } shadow-sm`}>
                        <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                            <div className="flex-1">
                                <div className="text-xs sm:text-sm font-bold tracking-wide uppercase text-gray-500 mb-1">
                                    Official Recommendation
                                </div>
                                <h3 className="text-2xl sm:text-4xl font-extrabold text-gray-900 mb-2">
                                    {rec.selected_crop}
                                </h3>

                                {rec.economic_switch && (
                                    <span className="inline-flex items-center gap-1 px-2 sm:px-3 py-1 rounded-full bg-green-100 text-green-800 text-xs font-bold border border-green-200 mb-3">
                                        <Coins className="w-3 h-3" /> Profit Optimized (+30% ROI)
                                    </span>
                                )}
                            </div>
                            <div className="text-left sm:text-right">
                                <div className="text-2xl sm:text-3xl font-bold text-green-700">{rec.confidence}%</div>
                                <div className="text-xs text-gray-500">Confidence Score</div>
                            </div>
                        </div>

                        <div className="mt-4 p-3 sm:p-4 bg-white rounded-lg border border-gray-100">
                            <h4 className="font-bold text-sm sm:text-base text-gray-700 mb-1 flex items-center gap-2">
                    ...
                    <div>
                        <h4 className="text-xs sm:text-sm font-bold text-gray-700 mb-2 mt-4 ml-1 uppercase">Select Crop to View Strategy:</h4>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                            {rec.top_candidates.map((cand, idx) => (
                                <div
                                    key={idx}
                                    onClick={() => setSelectedCandidateIndex(idx)}
                                    className={`p-3 sm:p-4 rounded-lg border cursor-pointer transition-all transform active:scale-95 sm:hover:scale-105 ${idx === selectedCandidateIndex
                                ...
                                >
                                    <div className="flex justify-between items-center mb-2">
                                        <div className="flex flex-col">
                                            <span className={`font-bold text-sm sm:text-base ${idx === selectedCandidateIndex ? 'text-green-800' : 'text-gray-800'}`}>
                                                #{idx + 1} {cand.crop}
                                            </span>
                                            {cand.crop === rec.selected_crop && <span className="text-[10px] text-green-700 font-bold uppercase">Officially Recommended</span>}
                                        </div>
                                        <span className="text-xs sm:text-sm font-mono text-gray-500">{cand.probability.toFixed(1)}%</span>
                                    </div>
                    ...
                    <div className="animate-fade-in relative">
                        ...
                        <div className="mb-4 flex items-center justify-center">
                            <span className="bg-gray-800 text-white px-3 sm:px-4 py-1 rounded-full text-xs font-bold uppercase tracking-wider shadow-md">
                        ...
                        {/* FINANCIAL LEDGER */}
                        {financials && (
                            <div className="bg-gray-800 text-white p-4 sm:p-6 rounded-2xl shadow-md mb-6">
                                <h4 className="text-base sm:text-lg font-bold mb-4 flex items-center gap-2">
                                    <TrendingUp className="w-4 h-4 sm:w-5 sm:h-5 text-green-400" /> Financial Forecast ({(financials as any).area_acres || 5} acres)
                                </h4>
                                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 text-center">
                                    <div>
                                        <div className="text-xs text-gray-400 uppercase font-bold">Est. Cost</div>
                                        <div className="text-lg sm:text-xl font-mono text-white">₹{(financials as any).estimated_cost.toLocaleString()}</div>
                                    </div>
                                    <div>
                                        <div className="text-xs text-slate-400 uppercase font-bold">Revenue</div>
                                        <div className="text-lg sm:text-xl font-mono text-green-400">₹{(financials as any).projected_revenue.toLocaleString()}</div>
                                    </div>
                                    <div>
                                        <div className="text-xs text-gray-400 uppercase font-bold">Net Profit</div>
                                        <div className="text-xl sm:text-2xl font-mono font-bold text-green-400">₹{(financials as any).net_profit.toLocaleString()}</div>
                                    </div>
                                    <div>
                                        <div className="text-xs text-gray-400 uppercase font-bold">ROI</div>
                                        <div className="text-lg sm:text-xl font-mono text-green-400">{(financials as any).roi_percentage}%</div>
                                    </div>
                                </div>
                            </div>
                        )}
