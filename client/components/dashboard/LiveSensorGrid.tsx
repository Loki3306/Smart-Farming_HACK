...
const SensorCard: React.FC<SensorCardProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Icon */}
      <div
        className={`inline-flex p-3 rounded-xl bg-white/20 backdrop-blur-md mb-4 ${color}`}
      >
        {icon}
      </div>
...
export const LiveSensorGrid: React.FC = () => {
  ...
  const npkPercentage = sensorData
    ? ((sensorData.npk / 1023) * 100).toFixed(1)
    : "0.0";
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <motion.div
          ...
          transition={{
            repeat: systemStatus.isOnline ? Infinity : 0,
            duration: 2,
          }}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <SensorCard
          ...
          color="text-white"
          gradient={
            (sensorData?.soil_ph ?? 7) > 7.5
              ? "from-red-500/90 to-red-600/90" // Alkaline Alert
              : (sensorData?.soil_ph ?? 7) < 5.8
                ? "from-amber-500/90 to-amber-600/90" // Acidic Alert
      ...
      {sensorData && (
        ...
        >
          Last updated:{" "}
          {new Date(
            sensorData.timestamp || systemStatus.lastUpdate,
          ).toLocaleTimeString()}
        </motion.div>
      )}
