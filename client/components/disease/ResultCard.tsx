...
export default function ResultCard({
  ...
  if (result.status === "unsupported_crop") {
    ...
    const displayCrop =
      Object.keys(MODEL_CROP_MAP).find(
        (k) => MODEL_CROP_MAP[k] === modelCrop,
      ) || modelCrop;
        ...
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-muted-foreground">
              {t("result.unsupportedCrop")}
            </h3>
            <p className="mt-2 text-sm text-muted-foreground">
              {t("result.unsupportedDesc")} <strong>{displayCrop}</strong>
            </p>
            <p className="mt-2 text-sm text-muted-foreground">
              {t("result.supportedCrops")}{" "}
              {SUPPORTED_DISPLAY_CROPS.slice(0, 6).join(", ")}
              {SUPPORTED_DISPLAY_CROPS.length > 6 ? ", ..." : ""}.
            </p>
          </div>
          <div>
            <button
              className="inline-flex items-center rounded bg-gray-200 px-3 py-1 text-sm"
              onClick={onReset}
            >
              {t("result.tryAnotherImage")}
            </button>
          </div>
        </div>
      </section>
  ...
  const rawDisease =
    result.disease ??
    (top?.class
      ? typeof top.class === "string"
        ? top.class.split("___")[1] || top.class
        : top.class
      : null);
  ...
  const confidenceLabel =
    ...
    (confidence >= 80 ? "sure" : confidence >= 50 ? "maybe" : "confused");
  ...
  const state =
    confidence >= 85 ? "confident" : confidence >= 50 ? "uncertain" : "none";
          ...
          <div className="relative flex items-center">
            ...
            <div className="absolute left-0 right-0 top-0 bottom-0 flex items-center justify-center">
              <div className="text-center">
                <div
                  className={`text-base font-semibold capitalize ${state === "confident" ? "text-emerald-700" : state === "uncertain" ? "text-amber-700" : "text-red-600"}`}
                >
                  {confidenceLabel}
                </div>
            ...
            <div className="mt-2 text-sm text-muted-foreground">
              {showLowConfidenceError
                ...
                : state === "confident"
                  ...
                  : "Review suggested steps"}
            </div>
            <div className="mt-3">
              <button
                className="inline-flex items-center rounded bg-gray-100 px-3 py-1 text-sm"
                onClick={onReset}
              >
                {t("result.checkAnotherLeaf")}
              </button>
            </div>
          </div>
        </div>
              ...
              result.predictions.length > 1 && (
                <div className="rounded-lg bg-emerald-50 p-3">
                  <div className="text-sm font-medium">
                    {t("result.otherCandidates")}
                  </div>
                  <ul className="mt-2 text-sm text-muted-foreground">
                    {result.predictions.map((p: any, i: number) => {
                      const raw = p.class?.includes("___")
                        ? p.class.split("___")[1]
                        : p.class;
                      return (
                        <li key={i}>
                          {formatDisease(raw)} —{" "}
                          {Math.round(p.confidence * 100 * 10) / 10}%
                        </li>
                      );
                    })}
                  </ul>
              ...
              <div>
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-xs text-muted-foreground">
                    Source: {diseaseInfo?.source || "chatbot"}
                  </div>
                </div>
                  ...
                  <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2 text-sm text-muted-foreground">
                    ...
                    {diseaseInfo.data.precautions && (
                      <div className="rounded-lg bg-gray-50 p-3 md:col-span-2">
                        ...
                        <ul className="mt-2 list-disc list-inside">
                          {diseaseInfo.data.precautions.map(
                            (p: string, i: number) => (
                              <li key={i}>{p}</li>
                            ),
                          )}
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="mt-2 text-xs text-amber-700">
                          💡 Note: AI model returned unstructured response.
                          Consider using a more advanced model for better
                          results.
                        </div>
                      </div>
                    </div>
                ...
                <div className="mt-4 text-xs text-muted-foreground">
                  Not a replacement for expert advice • Leaf-based analysis only
                </div>
              </div>
            )}
          </div>
