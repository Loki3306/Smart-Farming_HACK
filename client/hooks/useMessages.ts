...
export function useMessages(conversationId: string | null) {
  ...
  useEffect(() => {
    ...
    const unsubscribeNew = chatService.subscribeToMessages(
      conversationId,
      (newMessage) => {
        setMessages((prev) => {
          // Avoid duplicates
    ...
    const unsubscribeUpdate = chatService.subscribeToMessageUpdates(
      conversationId,
      (updatedMessage) => {
        setMessages((prev) =>
  ...
  const sendMessage = useCallback(
    async (content: string, receiverId: string, imageUrl?: string) => {
      if (!conversationId || !user?.id || (!content.trim() && !imageUrl))
        return;
