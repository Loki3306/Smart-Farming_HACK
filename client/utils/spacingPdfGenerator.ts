...
export const generatePlantingGuidePDF = (guide: any) => {
  ...
  doc.text(
    `Planting Guide: ${guide.crop_type.charAt(0).toUpperCase() + guide.crop_type.slice(1)}`,
    15,
    12,
  );
  doc.setFontSize(10);
  ...
  doc.text(
    `Farm Size: ${guide.farm_layout.farm_size_hectares} hectares`,
    120,
    35,
  );
  ...
  const spacingData = [
    ...
    [
      "Plants per Hectare",
      guide.farm_layout.plants_per_hectare.toLocaleString(),
    ],
    ["Total Plants Needed", guide.farm_layout.total_plants.toLocaleString()],
    [
      "Field Dimensions",
      `${guide.farm_layout.field_dimension_m}m x ${guide.farm_layout.field_dimension_m}m`,
    ],
  ];
  ...
  const seedData = [
    [
      "Seed Required",
      `${guide.seed_requirements.seed_required} ${guide.seed_requirements.unit}`,
    ],
