  ...
  const patterns = [
    {
      ...
      transform: (match: RegExpMatchArray) => ({
        mode: t(`control.${match[1].toLowerCase()}`) || match[1],
      }),
    },
    {
...
export const getLocalizedAction = (
  action: string,
  t: (key: string) => string,
) => {
  // Map known action titles
  if (action.includes("Irrigation Cycle"))
    return t("actionLog.types.irrigation");
  if (action.includes("Manual Irrigation"))
    return t("control.manualIrrigation");
  if (action.includes("Fertilization"))
