...
export const yieldService = {
  ...
  async getOptimizationTips(
    ...
    if (soilMoisture !== undefined)
      params.set("soil_moisture", String(soilMoisture));
    ...
    const response = await fetch(
      `${API_BASE}/yields/optimize/${cropType}?${params}`,
    );

    if (!response.ok) {
      throw new Error(
        `Failed to get optimization tips: ${response.statusText}`,
      );
    }
  ...
  async getBenchmarks(
    cropType: string,
    region?: string,
  ): Promise<{ benchmarks: YieldBenchmark[]; source: string }> {
    const params = region ? `?region=${encodeURIComponent(region)}` : "";
    const response = await fetch(
      `${API_BASE}/yields/benchmark/${cropType}${params}`,
    );
  ...
  async getYields(
    ...
    const response = await fetch(
      `${API_BASE}/yields/farmer/${farmerId}?${params}`,
    );
  ...
  async getHistory(
    ...
    const response = await fetch(
      `${API_BASE}/yields/history/${farmerId}?${params}`,
    );
