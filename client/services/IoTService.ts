...
class IoTServiceClass {
  ...
  connect(farmId: string) {
    ...
    try {
      ...
      this.ws.onmessage = (event) => {
        ...
        console.log(
          "%c[IoTService] 📥 RAW WEBSOCKET DATA RECEIVED!",
        ...
        try {
          ...
          console.log(
            "%c[IoTService] 📨 PARSED MESSAGE:",
            "background: #0099ff; color: #fff; font-weight: bold; padding: 4px;",
            message,
          );
          ...
          if (
            ...
            console.log(
              "   ├─ Nitrogen calc: ",
              Math.round(sensorData.npk * 0.14),
            );
            console.log(
              "   ├─ Phosphorus calc: ",
              Math.round(sensorData.npk * 0.045),
            );
            console.log(
              "   └─ Potassium calc: ",
              Math.round(sensorData.npk * 0.2),
            );
            ...
            message.type === "agronomy_analysis" ||
            message.type === "AI_DECISION"
          ) {
            ...
            message.type === "leaching_triggered" ||
            message.type === "wind_safety_alert"
          ) {
            // Dispatch alerts
            window.dispatchEvent(
              new CustomEvent("iot-data", { detail: message }),
   ...
   */
  private notifyIrrigationCallbacks(event: {
    reason: string;
    timestamp: string;
  }) {
