 ...
 */
const REGIONAL_MULTIPLIERS: Record<
  string,
  { material: number; labor: number; name: string }
> = {
  // North India
  PB: { material: 1.15, labor: 1.25, name: "Punjab" }, // Higher wages, good infrastructure
  HR: { material: 1.1, labor: 1.2, name: "Haryana" }, // NCR proximity, higher costs
  ...
  JK: { material: 1.3, labor: 1.1, name: "Jammu & Kashmir" }, // Remote, higher transport

  // South India
  KA: { material: 1.0, labor: 1.05, name: "Karnataka" }, // Bangalore hub, baseline
  TN: { material: 1.05, labor: 1.1, name: "Tamil Nadu" }, // Urban centers nearby
  KL: { material: 1.2, labor: 1.3, name: "Kerala" }, // Highest wages, hilly terrain
  AP: { material: 0.98, labor: 0.95, name: "Andhra Pradesh" }, // Agricultural state, competitive
  TS: { material: 1.02, labor: 1.0, name: "Telangana" }, // Hyderabad market

  // West India
  MH: { material: 1.0, labor: 1.0, name: "Maharashtra" }, // BASELINE region
  GJ: { material: 1.05, labor: 1.08, name: "Gujarat" }, // Industrial state, good infra
  RJ: { material: 1.08, labor: 0.9, name: "Rajasthan" }, // Desert transport, lower wages
  GA: { material: 1.15, labor: 1.25, name: "Goa" }, // Small state, tourism economy

  // East India
  WB: { material: 0.9, labor: 0.88, name: "West Bengal" }, // Competitive market
  BR: { material: 0.85, labor: 0.75, name: "Bihar" }, // Lowest costs
  JH: { material: 0.88, labor: 0.8, name: "Jharkhand" }, // Developing state
  ...
  CG: { material: 0.9, labor: 0.85, name: "Chhattisgarh" }, // Lower development

  // Northeast (higher due to transport)
  AS: { material: 1.18, labor: 0.95, name: "Assam" }, // Remote, transport costs
  NE: { material: 1.25, labor: 1.0, name: "Northeast States" }, // Generic for other NE states
};
...
export const getCurrentRegion = (): {
  code: string;
  name: string;
  multipliers: { material: number; labor: number };
} => {
  const region =
    REGIONAL_MULTIPLIERS[currentRegion] || REGIONAL_MULTIPLIERS["MH"];
  return {
    code: currentRegion,
    name: region.name,
    multipliers: {
 ...
 */
export const getAllRegions = (): Array<{
  code: string;
  name: string;
  material: number;
  labor: number;
...
const getRegionalPrice = (
  ...
  const multiplier =
    REGIONAL_MULTIPLIERS[currentRegion] || REGIONAL_MULTIPLIERS["MH"];
  return Math.round(
    basePrice * (isLabor ? multiplier.labor : multiplier.material),
  );
};
...
export const getRegionalPricing = (): typeof BASE_PRICING => {
  ...
  for (const [key, value] of Object.entries(BASE_PRICING)) {
    ...
    regionalPricing[key] = Math.round(
      value * (isLabor ? multiplier.labor : multiplier.material),
    );
  }
...
export const getIrrigationRecommendations = (
  ...
  const dailyWaterLiters =
    (CROP_WATER_REQUIREMENTS[cropType] || CROP_WATER_REQUIREMENTS.default) *
    areaAcres;
  ...
  // 2. Sprinkler System
  const sprinklerScore = calculateSprinklerScore(
    distance,
    cropType,
    waterSource.type,
  );
...
const createDripRecommendation = (
  ...
  const components: IrrigationComponent[] = [
    {
      name: "HDPE Main Pipe",
      quantity: Math.ceil(mainlineLength),
      unit: "m",
      unitPrice: P.hdpePipe,
      totalPrice: Math.ceil(mainlineLength) * P.hdpePipe,
    },
    {
      name: "Drip Lateral Pipe",
      quantity: Math.ceil(lateralLength),
      unit: "m",
      unitPrice: P.dripLateralPipe,
      totalPrice: Math.ceil(lateralLength) * P.dripLateralPipe,
    },
    {
      name: "Drip Emitters",
      quantity: emitterCount,
      unit: "pcs",
      unitPrice: P.dripEmitter,
      totalPrice: emitterCount * P.dripEmitter,
    },
    {
      name: "Drip Filter System",
      quantity: 1,
      unit: "set",
      unitPrice: P.dripFilter,
      totalPrice: P.dripFilter,
    },
    {
      name: pumpName,
      quantity: 1,
    ...
    },
    {
      ...
      quantity: 1,
      unit: "unit",
      unitPrice: P.footValve,
      totalPrice: P.footValve,
    },
    {
      name: "Gate Valves",
      quantity: Math.ceil(areaAcres * 2),
      unit: "pcs",
      unitPrice: P.gateValve,
      totalPrice: Math.ceil(areaAcres * 2) * P.gateValve,
    },
    {
      name: "Fittings & Connectors",
      quantity: 1,
      unit: "set",
      unitPrice: Math.ceil(areaAcres * 500),
      totalPrice: Math.ceil(areaAcres * 500),
    },
  ];
...
const createSprinklerRecommendation = (
  ...
  const components: IrrigationComponent[] = [
    {
      name: 'PVC Main Pipe (3")',
      quantity: Math.ceil(pipelineLength),
      unit: "m",
      unitPrice: P.pvcPipe3inch,
      totalPrice: Math.ceil(pipelineLength) * P.pvcPipe3inch,
    },
    {
      name: 'Sprinkler Pipe (2")',
      quantity: Math.ceil(areaAcres * 200),
      unit: "m",
      unitPrice: P.sprinklerPipe,
      totalPrice: Math.ceil(areaAcres * 200) * P.sprinklerPipe,
    },
    {
      name: "Sprinkler Heads",
      quantity: sprinklerCount,
      unit: "pcs",
      unitPrice: P.sprinklerHead,
      totalPrice: sprinklerCount * P.sprinklerHead,
    },
    {
      name: "Sprinkler Risers",
      quantity: sprinklerCount,
      unit: "pcs",
      unitPrice: P.sprinklerRiser,
      totalPrice: sprinklerCount * P.sprinklerRiser,
    },
    {
      name: pumpName,
      quantity: 1,
    ...
    },
    {
      ...
      quantity: 1,
      unit: "unit",
      unitPrice: P.nonReturnValve,
      totalPrice: P.nonReturnValve,
    },
    {
      name: "Fittings & Connectors",
      quantity: 1,
      unit: "set",
      unitPrice: Math.ceil(areaAcres * 800),
      totalPrice: Math.ceil(areaAcres * 800),
    },
  ];
  ...
  const components: IrrigationComponent[] = [
    {
      ...
      quantity: Math.ceil(canalLength),
      unit: "m",
      unitPrice: 150,
      totalPrice: Math.ceil(canalLength) * 150,
    },
    {
      name: "Canal Lining (optional)",
      quantity: Math.ceil(canalLength * 0.5),
      unit: "m",
      unitPrice: 200,
      totalPrice: Math.ceil(canalLength * 0.5) * 200,
    },
    {
      name: "Field Leveling",
      quantity: Math.ceil(areaAcres),
      unit: "acre",
      unitPrice: 3000,
      totalPrice: Math.ceil(areaAcres) * 3000,
    },
    {
      name: "Bund Construction",
      quantity: Math.ceil(areaAcres * 100),
      unit: "m",
      unitPrice: 50,
      totalPrice: Math.ceil(areaAcres * 100) * 50,
    },
    {
      name: "Gate/Sluice",
      quantity: Math.ceil(areaAcres * 2),
      unit: "pcs",
      unitPrice: 500,
      totalPrice: Math.ceil(areaAcres * 2) * 500,
    },
  ];
...
const createPumpRecommendation = (
  ...
  const components: IrrigationComponent[] = [
    {
      name: pipeName,
      quantity: Math.ceil(distance * 1.1),
      unit: "m",
      unitPrice: pipeCost,
      totalPrice: Math.ceil(distance * 1.1) * pipeCost,
    },
    {
      name: pumpName,
      quantity: 1,
    ...
    },
    {
      ...
      unit: "pcs",
      unitPrice: P.gateValve,
      totalPrice: 3 * P.gateValve,
    },
    {
      name: "Elbows & Tees",
      quantity: Math.ceil(distance / 50),
      unit: "set",
      unitPrice: P.elbow + P.tee,
      totalPrice: Math.ceil(distance / 50) * (P.elbow + P.tee),
    },
    {
...
export const createIrrigationPlan = (
  ...
  const distance = calculateDistance(
    sectionLat,
    sectionLng,
    waterSource.coordinates[0],
    waterSource.coordinates[1],
  );
  ...
  if (distance > 1000)
    warnings.push("Long distance may increase operational costs");
  if (waterSource.type === "pond" && section.area > 5)
    warnings.push("Pond may not have sufficient capacity for large area");
  if (selectedMethod === "surface" && cropType !== "Rice")
    warnings.push("Surface irrigation is less efficient for this crop");
  if (recommendation.efficiency < 60)
    warnings.push("Consider a more efficient irrigation method");
  ...
  if (selectedMethod === "drip")
    implementationRecs.push("Install sand/disc filter to prevent clogging");
  if (selectedMethod === "sprinkler")
    implementationRecs.push(
      "Check prevailing wind direction for sprinkler placement",
    );
  if (distance > 500)
    implementationRecs.push(
      "Consider installing booster pump for consistent pressure",
    );
