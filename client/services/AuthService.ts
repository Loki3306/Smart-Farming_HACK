...
class AuthServiceClass {
  ...
  async login(payload: LoginPayload): Promise<AuthResponse> {
    ...
    if (supabase) {
      ...
      console.log(
        "[Login] Query result:",
        userData ? `Found user with phone: ${userData.phone}` : "No user found",
      );
      ...
      if (error) {
        ...
        if (error.code === "PGRST116") {
          throw new Error(
            "Database error: Multiple accounts found. Please contact support.",
          );
        }
      ...
      if (farmError) {
        console.warn(
          "[Login] Farm lookup error (treating as not onboarded):",
          farmError,
        );
      }
      ...
      console.log(
        "[Login] Session stored. Auth token:",
  ...
  async getCurrentUser(): Promise<User | null> {
    ...
    if (supabase) {
      ...
      console.log(
        "[Auth] Cache miss, loading from Supabase for userId:",
        userId,
      );
      const { data: userData, error } = await supabase
        .from("farmers")
      ...
      if (farmError) {
        console.warn(
          "[Auth] Farm lookup error (treating as not onboarded):",
          farmError,
        );
      }
      ...
      console.log(
        "[Auth] User loaded from Supabase and cached:",
        user.fullName,
      );
