...
export async function getCourseById(
  id: string,
): Promise<ApiResponse<Course & { lessons: any[] }>> {
...
export async function getCourseLessons(
  ...
  return apiRequest(
    `/learn/courses/${courseId}/lessons?page=${page}&limit=${limit}`,
  );
}
...
export async function completeCourse(courseId: string): Promise<
  ApiResponse<{
    enrollment: Enrollment;
    badge: Badge | null;
    message: string;
...
> {
  return apiRequest(
    `/learn/search?q=${encodeURIComponent(query)}&limit=${limit}`,
  );
}
...
export async function getLearningDashboard(): Promise<
  ApiResponse<{
    stats: LearningStats;
