...
class CropPriceService {
  ...
  getImageFallbacks(commodity: string, imageNumber: number): string[] {
    ...
    return extensions.map(
      (ext) => `/Agricultural-crops/${folder}/image (${imageNumber}).${ext}`,
    );
  }
  ...
  async getCropDataForMarketplace(): Promise<CropData[]> {
    ...
    const cropsWithImages = prices.filter(
      (price) => this.mapCommodityToFolder(price.commodity) !== null,
    );
    ...
    commodityMap.forEach((crops, commodityKey) => {
      ...
      crops.slice(0, 3).forEach((crop, index) => {
        ...
        cropData.push({
          ...
          arrivalDate:
            crop.arrival_date || new Date().toISOString().split("T")[0],
  ...
  private getSeedsData(): CropData[] {
    const seeds = [
      {
        name: "Rice",
        varieties: ["Basmati", "IR-64", "Pusa", "Swarna"],
        price: 3000,
        unit: "kg",
      },
      {
      ...
      {
        name: "Maize",
        varieties: ["Hybrid", "Sweet Corn", "Dent Corn"],
        price: 1500,
        unit: "kg",
      },
      {
        name: "Cotton",
        varieties: ["Bt Cotton", "Hybrid", "Desi"],
        price: 4500,
        unit: "kg",
      },
      {
        name: "Soybean",
        varieties: ["JS-95-60", "NRC-37", "Punjab-1"],
        price: 3500,
        unit: "kg",
      },
      {
        name: "Tomato",
        varieties: ["Hybrid F1", "Pusa Ruby", "Desi"],
        price: 2500,
        unit: "kg",
      },
      {
        name: "Chilli",
        varieties: ["G4", "Guntur", "Kashmiri"],
        price: 5000,
        unit: "kg",
      },
      {
        name: "Sunflower",
        varieties: ["Hybrid", "KBSH-44", "Morden"],
        price: 2800,
        unit: "kg",
      },
    ];
